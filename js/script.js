...
(() => {
  const progressPath = document.querySelector('.scroll-progress-wrap svg path');
...
/* Project Gallery Slider JS Logic (Lazy Loaded 101 Images) */
(() => {
  ...
  if (!wrapper || !counter || !progressFill) return;

  const totalImages = 101;
  const slides = [];

  // Remove the placeholder
  const placeholder = document.getElementById('projectSlidePlaceholder');
  if (placeholder) placeholder.remove();

  // Create slide elements programmatically to avoid heavy HTML payload
  for (let i = 1; i <= totalImages; i++) {
    const slide = document.createElement('div');
    slide.className = 'project-slide';
    
    const img = document.createElement('img');
    img.dataset.src = `images/projects/IMG (${i}).jpg`;
    img.alt = `Dokumentasi Proyek ${i}`;
    img.loading = 'lazy';
    
    // Set fallback onerror
    img.onerror = function() {
      this.src = `https://placehold.co/800x450/171717/ffffff?text=Dokumentasi+Proyek+${i}`;
    };

    const caption = document.createElement('div');
    caption.className = 'project-caption';
    caption.textContent = `Dokumentasi Kegiatan & Proyek [Foto ${i} dari ${totalImages}]`;
  ...
  // Load image source on demand (lazy loading)
  const lazyLoadImage = (idx) => {
  ...
  // Pre-load neighboring slides to keep browsing instant
  const preloadNeighboringSlides = (idx) => {
    lazyLoadImage(idx);
    lazyLoadImage((idx + 1) % slides.length);
    lazyLoadImage((idx - 1 + slides.length) % slides.length);
  };
  ...
  const showSlide = (idx) => {
    ...
    currentIdx = idx;
    preloadNeighboringSlides(currentIdx);
    ...
    const percentage = ((currentIdx + 1) / slides.length) * 100;
  ...
  const startAutoplay = () => {
    timer = setInterval(nextSlide, 6000); // Auto-slide every 6 seconds
  };
